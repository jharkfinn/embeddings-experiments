...
DEFAULT_ANALYSIS_APP = "kv-prepend-analysis-cpu"
...
def _deploy_cpu_app(app_name: str) -> None:
    subprocess.run(
        [
            "cerebrium",
            "deploy",
...
def main() -> int:
    ...
    parser.add_argument("--analysis-app", default=DEFAULT_ANALYSIS_APP)
    ...
    analysis_run_id = args.analysis_run_id or f"{time.strftime('%Y%m%d_%H%M%S')}_{uuid.uuid4().hex[:8]}"

    _deploy_cpu_app(args.analysis_app)

    endpoint = (
        f"https://api.aws.{region}.cerebrium.ai/v4/"
        f"{urllib.parse.quote(project)}/{urllib.parse.quote(args.analysis_app)}/"
        f"analyze_latest_calibration?async=true"
    )
    ...
    result = {
        "request": response,
        "endpoint": endpoint,
        "analysis_app": args.analysis_app,
        "storage_app": args.storage_app,
        "calibration_run_id": calibration_run_id,
        "analysis_run_id": analysis_run_id,
        "status_remote_path": (
            f"{args.storage_app}/calibration_runs/{calibration_run_id}/"
            f"analysis_runs/{analysis_run_id}/status.json"
        ),
        "log_remote_path": (
            f"{args.storage_app}/calibration_runs/{calibration_run_id}/"
            f"analysis_runs/{analysis_run_id}/analysis_run.log"
        ),
        "output_remote_path": (
            f"{args.storage_app}/calibration_runs/{calibration_run_id}/"
            f"analysis_runs/{analysis_run_id}/capture_analysis.json"
        ),
    }
