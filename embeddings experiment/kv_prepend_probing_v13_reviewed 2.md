...
**Storage: fp8** for all collected vectors (z_attn, router_logits, h_pre_moe, V_raw). Top-k expert indices stored as int8 (values 0-127). β stored in bf16. Calibration subset retains bf16 for diagnostic analyses near routing boundaries. **Quantization is an evaluation-time decision, not a collection-time commitment.** Trinary {-1, 0, +1}, positive-only trinary {0, +1}, top-k indicators, and other encoding schemes are applied at retrieval evaluation time from the fp8 source. This enables comparing signed vs. positive-only trinary for router logits (negative values may be noise for routers but informative for attention) and any other scheme without rerunning the forward pass. Prompt scaffold tokens from the compression template should be masked out of all retrieval representations, token-collapse metrics, and semantic-abstraction evaluations.
...
2. Pre-attention residual stream resid_pre_attn (decoder-layer input before input_layernorm; needed for exact RMSNorm reconstruction)
...
## Early experimental findings (SciFact, binary vectors, echo on Qwen 3.5)
...
Bias-spectrum computed online during forward pass on calibration subset only (~17% extra compute on ~100 docs): sweep ~50 bias values per layer in headwise pre-o_proj space using live model weights (W_O, gate, layernorm); store compact signatures (~8 floats per token per layer: sensitivity, curvature, first phase transition, flip count, margin, KL) plus sparse transition records (b_critical, expert_out, expert_in, rank_position for each top-k change; typically 2-5 per token, negligible storage). No weight extraction needed. Routing-trace entropy and cross-layer MI under both conditions. Expert-output distinctness at flip points. Intrinsic dimensionality on causal m, causal d, propagated d, router distributions, value aggregates separately.
...
2. **Single-slot null effects are bottlenecked-null effects.** A near-zero treatment effect means the chosen summary slot did not carry the needed information; it does not imply that future-aware context would be useless. Multi-slot follow-up remains a core disambiguation step.

3. **Bridge fairness matters.** Echo vs. KV-prepend comparisons should match evaluation format as closely as possible, including prompt scaffolding, token masking, quantization protocol, and retrieval operator. Otherwise differences may reflect prompt format rather than extraction mechanics.
