...
|---|--------|--------------|-----------------|-----------|
| 1 | `resid_pre_attn` (decoder-layer input residual) | Decoder-layer input, **before** input_layernorm | (n, 2048) | fp16/bf16 |
| 2 | `Q_pre_rope` | After Q projection and q_norm, before RoPE | (n, 32, d_head) | calibration subset only |
| 3 | `V_raw` | After V projection, before attention | (n, 4, d_head) | fp16/bf16 |
| 4 | `A_causal_slice` | Post-softmax causal attention weights, but **only the slices actually needed** | see note | fp16/bf16 |
| 5 | `z_attn` | After `o_proj`, before residual add | (n, 2048) | fp16/bf16 |
| 6 | `h_pre_moe` | After attention residual add + post-attention RMSNorm | (n, 2048) | fp16/bf16 |
| 7 | `router_logits_pre_softmax` | Inside router, before softmax | (n, 128) | bf16/fp32 |
| 8 | `top_k_indices` | After top-k selection | (n, 8) | int16 |
| 9 | `final_token_K_raw` | Last-token K before RoPE, per KV group | (4, d_head) | fp16/bf16 |
| 10 | `final_token_K_rot` | Last-token K after RoPE, per KV group | (4, d_head) | fp16/bf16 |
| 11 | `final_token_V` | Last-token V per KV group | (4, d_head) | fp16/bf16 |
...
**Task:** Implement trinary quantization carefully enough that threshold semantics are unambiguous.

The original draft used a percentile directly on `abs(x)`, which can be misread. Use one of these explicit parameterizations:

- `nonzero_fraction`: keep this fraction of dimensions nonzero
- `threshold`: use a fixed threshold calibrated from a reference distribution

A safer helper is:

```python
def trinarize_by_fraction(x: torch.Tensor, nonzero_fraction: float) -> torch.Tensor:
    # Example: nonzero_fraction=0.1 keeps roughly the largest 10% by |x|
...
- For router features, do not assume the bottom-k negative logits are as useful as the positive top-k. Compare:
  - full signed trinary logits
  - top-k indicator
  - ordered top-k + margins
  - pooled route motifs

Keep float/bf16 versions on a matched evaluation slice, not only on tiny calibration examples.
...
**Single-vector**
- mean or hybrid pooled
- both float/bf16 and trinary
...
This part needs a more exact formula than the original sketch.
...
- **Quantization confounds:** keep a matched higher-precision slice throughout the pipeline.
